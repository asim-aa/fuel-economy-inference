...
kw_pvals <- function(y, g, B = 2000) {
  k <- nlevels(g)
  
  p_asym <- pvalue(kruskal_test(y ~ g, distribution = asymptotic()))
  p_mc   <- pvalue(kruskal_test(y ~ g, distribution = approximate(B = B)))
...
for (i in seq_len(R)) {
  ...
  pvals_null[i, ] <- kw_pvals(dat$y, dat$g, B = B)
}
...
for (i in seq_len(R)) {
  ...
  pvals_alt[i, ] <- kw_pvals(dat$y, dat$g, B = B)
}
